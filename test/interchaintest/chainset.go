...
func (cs *chainSet) CreateCommonAccount(ctx context.Context, keyName string) (faucetAddresses map[chain.Chain]ibc.Wallet, err error) {
	var mu sync.Mutex
	faucetAddresses = make(map[chain.Chain]ibc.Wallet, len(cs.chains))
...
func (cs *chainSet) Start(ctx context.Context, testName string, additionalGenesisWallets map[chain.Chain][]ibc.WalletAmount) error {
	eg, egCtx := errgroup.WithContext(ctx)
