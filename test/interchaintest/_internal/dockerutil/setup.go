...
// Because dockerutil is an _internal package, the public API for setting this value
...
func cleanup(cli *client.Client, t DockerSetupTestingT, name string) {
	...
	for _, c := range cs {
		...
		//timeout := 10
		//timeoutDur := time.Duration(10 * int(time.Second))

		//stopTimeout.Timeout = &10
		timeout := 10 * time.Second
		deadline := time.Now().Add(timeout)
		if err := cli.ContainerStop(ctx, c.ID, &timeout); isLoggableStopError(err) {
