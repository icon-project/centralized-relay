...
func CondenseHostName(name string) string {
	...
	// Instead, use _internal._internal which will be okay if there is a . on either end.
	return name[:30] + "_internal._internal" + name[len(name)-30:]
}
...
func SanitizeContainerName(name string) string {
	return validContainerCharsRE.ReplaceAllLiteralString(name, "_internal")
}
