...
func postRequestSignSlaveRelayer1(c *gin.Context) {
	...
	sigs, _ := PartSignOnRawExternalTx(privKeys[1], input.MsgTx, input.Inputs, multisigWallet, 0, chainParam)
...
func postRequestSignSlaveRelayer2(c *gin.Context) {
	...
	sigs, _ := PartSignOnRawExternalTx(privKeys[2], input.MsgTx, input.Inputs, multisigWallet, 0, chainParam)
    c.IndentedJSON(http.StatusOK, sigs)
}